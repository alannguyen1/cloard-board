...
_render_footer() {
  ...
  if [[ "${_view_mode:-kanban}" == "list" ]]; then
    if [[ ${_split_active:-0} -eq 1 ]]; then
      printf "${C_DIM}  j/k: nav  Enter/l: focus  D: done  H: history  Tab: filter  b: full list  q: quit${C_RESET}"
    else
      printf "${C_DIM}  j/k: nav  Tab: filter  Enter: open  </> status  :/\" reorder  t: rename  s: shell  D: done  H: history  c: new  b: split  v: kanban  q: quit${C_RESET}"
    fi
    ...
    printf "${C_DIM}  Tab: filter  j/k: cards  Enter: open  r: reopen  </> move  :/\" reorder  c: new  t: rename  s: shell  D: done  H: history  v: list  q: quit${C_RESET}"
  fi
}
